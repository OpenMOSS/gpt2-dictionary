...
def load_sae(dict_name):
    cfg = SAEConfig(
        **SAEConfig.get_hyperparameters(dict_name, "/remote-home/share/research/mechinterp/gpt2-dictionary/circuit-results", "final.pt", True),
        device = device,
    )

    sae = SparseAutoEncoder(cfg=cfg)
    sae.load_state_dict(torch.load(cfg.sae_from_pretrained_path, map_location=cfg.device)["sae"])
    return sae
...
class HookedTransformer(HookedRootModule):
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            sae_predicted_input_hat = self.L0RPr_sae(residual)[1][1]['x_hat']
            error = residual - sae_predicted_input_hat
            residual = sae_predicted_input_hat + error.detach()
            self.h = sae_predicted_input_hat.register_hook(lambda grad: self.L0RPr_grad.copy_(grad))
